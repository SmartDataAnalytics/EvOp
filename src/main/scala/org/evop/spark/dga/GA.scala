...
class GA(  f:  (Array[Gene],Double)  =>  Double  , //init:Initializer  ,
  ...
  var Threshold:Double  =  1.toDouble  /  (CL.toDouble * PartitionsCount.toDouble  )
  def minC(s1: Chromosome, s2: Chromosome): Chromosome = if (s1.fitness < s2.fitness) s1 else s2
  breakable{
  
    while(  !(  theStopper.stop(gens,0)  )  &&  condition  ==  true  )  {

        var Res1   =  theSelector.selection  (  chromoRDD  )
        var temp1  =  Res1.groupBy(_._1)
        var temp2  =  temp1.map  (  x  =>  (  x._1  ,  x._2.map(_._2)))
        var temp3  =  temp2.map(x=>(x._1,x._2.reduceLeft(minC)))
        
        //Res1.map(x=>x._1+"  - - -  "+x._2).foreach(println)
        //temp3.map(x=>x._1+"  - - - > > >  "+x._2).foreach(println)
        
        var Res2  =  temp3.map(  _._2.Genes  )
        var Res3  =  Res2.flatten
        
        var Results  =  new Chromosome(  0.0,  Res3.toArray,  f,  f(Res3.toArray,0.0)  )
  ...
  val fw = new FileWriter("/data/home/FahadMaqbool/DGA/newResults.txt", true)
  fw.write(fOutput)
  fw.close()
  ...
  val fDetail   = "\n\n\n"+Calendar.getInstance().getTime+"\nGens= "+ gens+",	Dimensions= "+CL+",	Func= "+TestFunctions.func+",	Population= "+P+",	Selector= "+SelectorType+",	Mutator= "+MutatorType+",	Replace= "+Replacement+",	Dir= "+direction+",	xovrP= "+
    ...
    //val outRDD  =  sc.parallelize(fOutput)
    //outRDD.saveAsTextFile("hdfs://172.18.160.17:54310/FahadMaqbool/DatasetX/"+gens+"-"+init.Dimensions+"-"+bdCastStrategy+"-"+bdCastSize+"-"+scala.util.Random.nextInt(1000)+".txt")
  val fw2 = new FileWriter("/data/home/FahadMaqbool/DGA/newDetails.txt", true)
  fw2.write(fOutput)
