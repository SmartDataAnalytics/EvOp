...
//  val Direction  =  sc.broadcast  (  Direct  )
  ...
  def selectBest  (  Partitioned: RDD[  (Double, Chromosome)  ]  )    :  (Double, Chromosome)  =  {
   ...
   def shareBest  (  SelectedBests: Array[  (Double, Chromosome)  ]    )  {
...
class RandomSelector(  sc:SparkContext  ,  MutatorType:String  ,  ReplaceScheme:String,  
    ...
    bdCastStrategy:String  ,  bdCastSize:Int  ,  PartitionsCount:Int) extends Selector   {
  ...
  override def selection(  Partitioned: RDD[  (Double, Chromosome)  ],  genGap:Int  =  1  
    ...
    val mapped  =  Partitioned.mapPartitionsWithIndex{
      ...
      (index, Iterator)  => {
        ...
        if (  bdStrategy  ==  "B2W"  )  {
          myArray  =  myArray.take(  myArray.length  -  k  )
          val recBDNext  =  recBD.filter(  _._1  ==  (  index+1  )  %  ptCount  )
          myArray  =  recBDNext  ++  myArray
        }
        ...
        //myArray  =  recBD  ++  myArray
        //println("First Element         " + myArray(0)._2  )
        var RandomNumber  =  scala.util.Random
        var B  =  myArray
        ...
        var OffSprings  = crossType match {
            ...
            //B  =  myArray.filter(_._2.fitness==0)
...
//            }
  ...
  override def selectBest  (  Partitioned: RDD[  (Double, Chromosome)  ]    )  :  (Double, Chromosome)  =  {
   ...
   var concatChromo  =  SelectedBests(0)._2.Genes
   for (  i  <-  1  to  SelectedBests.length  )  
     concatChromo  =  concatChromo  ++  SelectedBests(i)._2.Genes
     
   SelectedBests.foreach(println)
   bestSolutions.update(  SelectedBests    )
   
   //SelectedBests(0)
   (  0.0,  new Chromosome(concatChromo)  )  
  }
   
  
  override def eliminateWeak  (  Partitioned: RDD[  (Double, Chromosome)  ]  ,  
      PartitionsCount:Int): RDD[  (Double, Chromosome)]  =  {
    val Direction  =  Direct
    val bdStrategy  =  bdCastStrategy
    val k  =  bdCastSize
    var recBD  =  bestSolutions.value
